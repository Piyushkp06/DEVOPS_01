...
export const handleAlertmanagerWebhook = async (req, res, next) => {
  try {
    const { alerts, status } = req.body;

    if (!alerts || !Array.isArray(alerts)) {
      throw new ApiError(400, "Invalid payload from Alertmanager");
    }

    const createdIncidents = [];
    for (const alert of alerts) {
      if (alert.status === "firing") {
        // Try to match the service based on labels (e.g. app, service, job)
        const serviceName = alert.labels.service || alert.labels.app || alert.labels.job || "unknown";
        
        let service = await prisma.service.findFirst({
          where: { name: { equals: serviceName, mode: 'insensitive' } }
        });

        // Find a system user or admin to attribute the incident to
        let systemUser = await prisma.user.findFirst({
          where: { role: 'ADMIN' }
        });
        
        // Auto-create a system user if database is completely empty
        if (!systemUser) {
           ...
           if (!systemUser) {
             systemUser = await prisma.user.create({
               data: {
                 ...
                 email: "system@devops-platform.local",
        ...
        // Use a generic service if not found, or auto-create it!
        if (!service) {
        ...
        const incident = await prisma.incident.create({
          data: {
            title: `[ALERT] ${alert.labels.alertname || "Monitoring Alert"}`,
            description: alert.annotations.description || alert.annotations.summary || "No description provided from Alertmanager",
            severity: alert.labels.severity === "critical" ? "CRITICAL" : (alert.labels.severity === "warning" ? "HIGH" : "MEDIUM"),
            status: "OPEN",
        ...
        const keys = await redis.keys("incidents:*");
        if (keys.length > 0) {
    ...
    return res.status(200).json(new ApiResponse(200, createdIncidents, "Alerts processed successfully"));
  } catch (error) {
    next(error);
  }
};